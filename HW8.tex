...
to calculate the probability of survival, we use 

\[P(\text{survive}) = e^{-\lambda {t}}\]

therefore, the probability of failure is

\[P(\text{failure} = 1 - P(\text{survive}) = 1 - e^{-\lambda {t}}\]

\begin{enumerate}
\item hard drive rated at $300{,}000$ hr, so $p_h=1-e^{-1/300000}\approx \boxed{3.33\times 10^{-6}}$
\item graphics card rated at $120{,}000$ hr, so $p_g=1-e^{-1/120000}\approx \boxed{8.33\times 10^{-6}}$
\item power supply rated at $100{,}000$ hr, so $p_s=1-e^{-1/100000}\approx \boxed{1.00\times 10^{-5}}$
\item use probability of survival for this one, 
\[
    P_{\text{(survive for 5 hours)}}=e^{\Big(-5\Big(\tfrac{1}{300000}+\tfrac{1}{120000}+\tfrac{1}{100000}\Big)\Big)}
= e^{-\frac{5\cdot 13}{600000}}\approx \boxed{0.999892}
\]
\item say the entire computer is made up of only the hard drive, graphics card and a power supply, then 

    \[\lambda_{\text{sys}}=\tfrac{1}{300000}+\tfrac{1}{120000}+\tfrac{1}{100000}=\tfrac{13}{600000}\]

and as 

\[\lambda = \frac{1}{MTBF}\]

we can format this relation as

\[
    \text{MTBF}{\text{sys}}=\frac{1}{\lambda{\text{sys}}}=\boxed{\frac{600000}{13}\ \text{hr}}
\]
\end{enumerate}
\vspace{1em}
\hrulefill
...
we count all possible pairings, there's $4 \cdot 3$ pairs horizontally, and another $4 \cdot 3$ pairs vertically, so that's 24 pairs in total. 

\vspace{1em}

now, there's 4 possible configuration of the pair, let $B$ denotes boy, and $G$ denotes girl as 

\[G = \{BB, BG, GB, GG\}\]

the two configuration that will cause flirtation is $BG$ and $GB$, which each got a $\frac14$ probability, so 

\[P(\text{flirtation}) = \frac12\cdot\frac12 + \frac12\cdot\frac12 = \frac12\]

applies that to the amount of edges, $E(X) = 24 \cdot \frac12$, so $\boxed{12}$ flirtations is expected
\end{document}
