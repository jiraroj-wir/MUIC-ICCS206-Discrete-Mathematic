...
\solution
 ...
 \begin{enumerate}
      ...
      \[
      \text{if }r\text{ is rational, then }r^{1/5}\text{ is irrational}
      \]
      this can be easily disprove by counterexample; when $r = 2^5$
  (the base can be any rational number, since rational number holds
  closure of multiplication property), $r^{1/5} = (2^5)^{1/5} = 2$ ---
  which is a rational number, therefore the claim fails, contradiction
  happends, so if r is irrational, then $r^{1/5}$ will be irrational.
      \end{proof}
      \item the statement is $\boxed{\text{false}}$
      \begin{proof}
      we'll disprove this claim by counterexample: for
  some $n \in \mathbb{R}$, $r = n^{5/2} \implies r^{1/5} =
  \left(n^{5/2}\right)^{1/5} = n^{1/2}$ or $\sqrt{n}$ --- of which we
  know that at least $\sqrt{2}$ is an irrational number. so, the claim
  fails
      \end{proof}
      
        \item the statement is $\boxed{\text{true}}$
      \begin{proof}
      we assume for the sake of contradiction that
      \[
      \text{if }r^{1/5}\text{ is rational, then }r\text{ is irrational}
      \]
      this can be easily disprove by counterexample; when $r = 2^5$ we have
      $r^{1/5} = (2^5)^{1/5} = 2$ --- which is a rational number while $r$
      itself is also rational, therefore the claim fails, contradiction
      happends, so if $r^{1/5}$ is irrational, then $r$ is irrational.
      \end{proof}
  \end{enumerate}
...
\begin{proof}
  assume that the program don't make any mistake, so everyone always
  wins. the amount of money that each person will make in each rounds
  would be
  \[
  \frac{10000 \cdot 1000}{10000} = 1000 \text{ Baht}
  \]
  while the expense is also 1000 Baht --- it cancles out, so best case,
  no one is making any profits.
\end{proof}
...
\begin{proof}
if we just add up all the rugged area by the given rules
  \[
  \begin{aligned}
  \frac{9 \text{ rugs} \cdot 1\,\text{m}^2}{1 \text{ rug}} - \frac{9
  \text{ rugs} \cdot \tfrac{1}{9}\,\text{m}^2 \text{ (overlap)}}{1
  \text{ rug}} &= 8\,\text{m}^2\\
  &> 5\,\text{m}^2
  \end{aligned}
  \]
  this exceeds the room’s area, contradicting the assumption that all
  nine rugs fit without violating Intouch’s overlap constraint, so it
  is impossible to satisfy both requirements.
\end{proof}
...
\begin{proof}
  firstly, we factor the length of the square as
  \[
  x = p^2 - q^2 = (p+q)(p-q)
  \]
  now, we want x to be odd number, so we should be able to write x in a
  form of $x = 2k + 1$. checking all odd-even configurations for $p+q$
  and $p-q$ shows that the product is even whenever the factors are
  both even or both odd. however, noting that consecutive integers $k$
  and $k+1$ give
  \[
  (k + 1 - k)(k + 1 + k) = 1 \cdot (2k + 1) = 2k + 1
  \]
  we can choose $p = k+1$ and $q = k$, both in $I$, so every odd number
  $x$ can be constructed as the difference of two squares of integers.
\end{proof}
